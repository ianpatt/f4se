...
public:
	...
	DEFINE_MEMBER_FN(ModifyFlag, void, 0x00307990, UInt32 flag, bool doSet, bool unk1); // unk1 = 1 (appears to select which actor flags to use)
	DEFINE_MEMBER_FN(GetLevel, UInt16, 0x00307A80);
};
...
public:
	...
	DEFINE_MEMBER_FN(Get, void, 0x0030BCB0, BSString * out, TESForm * parent);
};
...
public:
	...
	public:
		...
		DEFINE_MEMBER_FN(ctor, void, 0x00765830, TESForm * a1, TESForm * a2, UInt64 unk1); // a1 might be player or subject, not sure yet
	};

	MEMBER_FN_PREFIX(Condition);
	// 
	DEFINE_MEMBER_FN(Evaluate, bool, 0x00767370, Evaluator * eval); // Evaluates only a single condition
};
...
public:
	...
	DEFINE_MEMBER_FN_4(AttachSkinnedObject, NiNode*, 0x0035BC00, NiNode* objectRoot, NiNode* parent, UInt32 bipedIndex, bool abFirstPerson);
};
