...
public:
	...
	const std::vector <uint8_t> & serial() const { return m_serial; }

private:
	bool getLeafCertSerial(const WCHAR * path, std::vector <uint8_t> * outSerial) const;
...
bool FileCertVerifier::getLeafCertSerial(const WCHAR * path, std::vector <uint8_t> * outSerial) const
{
	...
	std::vector <uint8_t> signerInfoBuf(signerInfoLen);
